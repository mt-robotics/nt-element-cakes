...
import type { CoffeeBean, TiramisuModel } from './tiramisu';

interface InteractionOptions {
  onCrack: () => void;
  onBeanHover: (label: string | null) => void;
}
...
export function createInteraction(rig: SceneRig, tiramisu: TiramisuModel, options: InteractionOptions): InteractionController {
  ...
  let dragging = false;
  let lastX = 0;
  let hoveredBean: CoffeeBean | null = null;
  ...
  function beanMeshes() {
    return tiramisu.coffeeBeans.flatMap((bean) => bean.mesh.children);
  }

  function findBean(object: THREE.Object3D): CoffeeBean | null {
    return tiramisu.coffeeBeans.find((bean) => object === bean.mesh || bean.mesh.children.includes(object)) || null;
  }
  ...
  window.addEventListener('pointermove', (event) => {
    ...
    const beanHit = raycaster.intersectObjects(beanMeshes(), false)[0];
    const bean = beanHit ? findBean(beanHit.object) : null;
    if (bean !== hoveredBean) {
      if (hoveredBean) hoveredBean.mesh.scale.setScalar(1);
      hoveredBean = bean;
      if (hoveredBean) {
        hoveredBean.mesh.scale.setScalar(1.22);
        options.onBeanHover(`${hoveredBean.platform} — tap to order`);
      } else {
        options.onBeanHover(null);
      }
    }
  });
  ...
  window.addEventListener('click', (event) => {
    ...
    const beanHit = raycaster.intersectObjects(beanMeshes(), false)[0];
    if (beanHit) {
      const bean = findBean(beanHit.object);
      if (bean) window.open(bean.url, '_blank', 'noopener,noreferrer');
      return;
    }
  ...
  const update = (delta: number) => {
    ...
    if (!dragging && !cracked) tiramisu.group.rotation.y += delta * 0.12;
    tiramisu.coffeeBeans.forEach((bean) => {
      bean.mesh.position.y = bean.baseY + Math.sin(performance.now() * 0.0017 + bean.index) * 0.09;
      bean.mesh.rotation.y += delta * (0.45 + bean.index * 0.08);
    });
  };
